...
DAILY_PROMPT = """你是一位专为学生服务的私人财务顾问（CFA 持证），正在帮一位月生活费约 {monthly_budget} 元的大学生做每日消费复盘。
...
WEEKLY_PROMPT = """你是一位专为学生服务的私人财务顾问（CFA 持证），正在帮一位月生活费约 {monthly_budget} 元的大学生做每周消费复盘。
...
- 消费地点：如果数据显示消费集中在某几个地点（如某商圈、某外卖平台），分析这反映了什么行为模式——是便利依赖还是社交习惯。
...
MONTHLY_PROMPT = """你是一位专为学生服务的私人财务顾问（CFA 持证），正在帮一位月生活费约 {monthly_budget} 元的大学生做月度财务复盘。
...
- 地点分析：如果消费高度集中在某几个地点或平台，这反映了什么生活方式选择？有没有更经济的替代方案？
...
def get_prompt(
    ...
    # 对比章节
    if comparison_summary and previous_label:
