...
- **🧠 记忆系统**：分层持久化关键洞察（用户画像 / 历史报告 / 滚动洞察），每次分析带上下文，越跑越懂你
...
├── location_resolver.py      # 🍜 高德地理编码 + 空间聚类（AOI→商圈→网格三层汇聚）
├── prompts.py                # AI 提示词模板（日报/周报/月报三套）
├── memory.py                 # 🧠 分层记忆系统（画像/洞察/报告快照）
...
| `insights_{mode}.json` | 各模式独立滚动洞察（daily/weekly/monthly） | 7/12/99 条 |
...
AI 生成报告
    ...
    └──▶ 截取报告洞察 → insights_{mode}.json（滚动累积）
